...
func (m Model) renderCompSyncHeader() string {
    ...
    if running > 0 { parts = append(parts, fmt.Sprintf("%d◯", running)) }
    if cancelled > 0 { parts = append(parts, fmt.Sprintf("%d✗", cancelled)) }
    if pending > 0 { parts = append(parts, fmt.Sprintf("%d◦", pending)) }
...
func (m Model) renderCompSyncFooter() string {
    left := ""
    if m.syncing { left = m.spinner.View() + " Synchronisiere..." }
    help := "ctrl+c: Abbrechen"
    line := help
    if left != "" { line = subtleStyle.Render(left) + "  |  " + help }
...
func (m *Model) updateCompSyncViewport() {
    ...
    for _, it := range m.compPre.items { if it.Run == RunDone { completed++ } else if it.Run == RunCancelled { cancelled++ } }
...
func (m Model) renderCompItemsList() string {
    ...
    if maxDisplay < 5 { maxDisplay = 5 }
    startIdx := 0
    if len(m.compPre.items) > maxDisplay {
        // anchor at first running or pending
        anchor := 0
        for i, it := range m.compPre.items { if it.Run == RunRunning || it.Run == RunPending { anchor = i; break } }
        startIdx = anchor - maxDisplay/2
        if startIdx < 0 { startIdx = 0 }
        if startIdx+maxDisplay > len(m.compPre.items) { startIdx = len(m.compPre.items) - maxDisplay }
    }
    endIdx := startIdx + maxDisplay
    if endIdx > len(m.compPre.items) { endIdx = len(m.compPre.items) }
    for i := startIdx; i < endIdx; i++ {
        it := m.compPre.items[i]
        status, color := m.getItemStatusDisplay(it.Run)
        action := it.State
        if it.CopyAsNew { action = "create" }
        line := fmt.Sprintf("%s %s (%s)", color.Render(status), it.Source.Name, subtleStyle.Render(action))
        if it.Issue != "" { line += " " + warnStyle.Render("["+it.Issue+"]") }
        b.WriteString(line+"\n")
    }
