...
func (m *mockStoryRawSyncAPI) UpdateStoryUUID(ctx context.Context, spaceID, storyID int, uuid string) error { return nil }
...
func TestSyncStory_Create_UsesRawPayload(t *testing.T) {
	...
	if len(api.rawCreates) != 1 { t.Fatalf("expected raw create, got %d", len(api.rawCreates)) }
	if len(api.typedCreates) != 0 { t.Fatalf("unexpected typed create") }
...
func TestSyncStory_Update_UsesRawPayload(t *testing.T) {
	...
	if len(api.rawUpdates) != 1 { t.Fatalf("expected raw update, got %d", len(api.rawUpdates)) }
	if len(api.typedUpdates) != 0 { t.Fatalf("unexpected typed update") }
...
func TestSyncFolder_Create_UsesRawPayload(t *testing.T) {
	...
	if len(api.rawCreates) != 1 { t.Fatalf("expected raw folder create, got %d", len(api.rawCreates)) }
